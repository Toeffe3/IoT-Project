...
#define E_UART0
  ...
  GPIO_PORTA_AFSEL_R |= (RX | TX); // set PA0 and PA1 to alternative function
								   // (RX and TX) (uart0)
...
uint32_t uart0_lcrh_stopbits (uint8_t stopbits) {
  if (stopbits == 2)
	return (1 << 3); // return bit 3 = 1
  else
	return 0; // return all zeros
}
...
//@formatter:on
void uart0_fifos_enable (void) { UART0_LCRH_R |= TX; }

void uart0_fifos_disable (void) { UART0_LCRH_R &= ~(RX); }

bool uart0_rx_ready (void) { return (UART0_FR_R & UART_FR_RXFF); }

char uart0_get_char (void) { return (UART0_DR_R); }

uint8_t uart0_get (void) { return (uint8_t) (UART0_DR_R - '0'); }

bool uart0_tx_ready (void) { return (UART0_FR_R & UART_FR_TXFE); }

void uart0_put (char ch) { UART0_DR_R = ch; }
...
void uart0_printnum (const uint8_t num) {
  while (!uart0_tx_ready ( ))
	;
  if (num >= 100)
	uart0_put (((num % 1000) / 100) + '0');
  if (num >= 10)
	uart0_put (((num % 100) / 10) + '0');
  if (num < 10)
	uart0_put ('0'); // Pad with '0'
  while (!uart0_tx_ready ( ))
	;
  uart0_put ((num % 10) + '0');
}
...
void uart0_printnum16 (const uint16_t num) {
  while (!uart0_tx_ready ( ))
	;
  if (num >= 100000)
	uart0_put (((num % 1000000) / 100000) + '0');
  while (!uart0_tx_ready ( ))
	;
  if (num >= 10000)
	uart0_put (((num % 100000) / 10000) + '0');
  while (!uart0_tx_ready ( ))
	;
  if (num >= 1000)
	uart0_put (((num % 10000) / 1000) + '0');
  while (!uart0_tx_ready ( ))
	;
  if (num >= 100)
	uart0_put (((num % 1000) / 100) + '0');
