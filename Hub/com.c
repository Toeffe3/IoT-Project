...
void com_init(void) {
	...
	SYSCTL_RCGC2_R |= SYSCTL_RCGC2_GPIOB;  // Enable the clock to the port by setting the appropriate bits
	vTaskDelay(10 / portTICK_RATE_MS);
	GPIO_PORTB_DIR_R &= ~(PIN_COM_0 | PIN_COM_1);  // A write a 1 = output and a write of a 0 = input.
	GPIO_PORTB_DEN_R |= (PIN_COM_0 | PIN_COM_1);  // The digital functions for the corresponding pin are enabled.
...
void com_send_signal(void) {
	...
	if ( signal_flag ) GPIO_PORTB_DIR_R |= (PIN_COM_0 | PIN_COM_1);
...
bool com_check_for_signal() {
	if ( com_get_data() == COM_SIGNAL ) return true;
	else return false;
}
...
void com_send_bit(bool bit, bool passthrough) {
	if ( !passthrough ) com_send_signal();

	if ( bit ) GPIO_PORTB_DATA_R |= PIN_COM_1;
	else GPIO_PORTB_DATA_R |= PIN_COM_0;
	vTaskDelay(COM_SPEED / portTICK_RATE_MS);
...
void com_task(void* pvParameters) {
	...
	while ( 1 ) {
		...
		switch ( com_mode ) {
			case COM_IDLE: {
				if ( xStreamBufferReceive(com_out, &data, 1, 250) == pdPASS ) str [len++] = *data;
				else if ( len > 0 ) com_mode = COM_WRITE;
				vTaskDelay(COM_SPEED / portTICK_RATE_MS);
				break;
			}
