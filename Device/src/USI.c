...
char twi_transmission(char *msg, char msg_size){
	...
	do{
		switch(USI_Master_State){
			...
			msg++;
			USI_SET_SDA_OUTPUT();
			if(msg==1) USIDR = 0xFF; // Load NACK to end transmission
	...
	while( !(PORT_USI & (1<<PIN_USI_SCL)) );  	// Wait for SCL to go high.
	USI_I2C_WAIT_HIGH();
	USI_SET_SDA_INPUT();            				// Release SDA.
	while( !(PORT_USI & (1<<PIN_USI_SDA)) );  	// Wait for SDA to go high.
